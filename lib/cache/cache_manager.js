...
import UpstashRedisClient, {
  isUpstashRedisRestApiUrl
...
function getCacheType() {
  if (hasRedis) {
    return isUpstashRedisRestApiUrl(BLOG.REDIS_URL) ? 'upstash' : 'redis'
  }
  if (enableLocalCache) return 'file'
  return 'memory'
}
...
function getCacheChain() {
  ...
  if (hasRedis) {
    chain.push({
      name: isUpstashRedisRestApiUrl(BLOG.REDIS_URL) ? 'upstash' : 'redis',
      api: isUpstashRedisRestApiUrl(BLOG.REDIS_URL)
        ? UpstashRedisClient
        : RedisCache
    })
  }

...
export async function getOrSetDataWithCustomCache(
  ...
  if (dataFromCache) {
    // 读取缓存时也尝试刷新写入缓存
    getDataFunction(...getDataArgs).then(data => {
      setDataToCache(key, data, customCacheTime)
    })
...
export function getApi() {
  if (BLOG.REDIS_URL) {
    if (isUpstashRedisRestApiUrl(BLOG.REDIS_URL)) {

...
/**
 * 判断是否是非Vercel环境，目前主要为了避免写入文件
 * @type {boolean}
 */
export const isNotVercelEnvironment = !BLOG['isVercel']
...
/**
 * 统计目标字符串在给定的字符串中出现的次数
 * @param str
 * @param target
 * @returns {number}
 */
export function countOccurrences(str, target) {
  if (!str || !target) {
    return 0
  }
...
const CDN_TRANSFORM = BLOG['CDN_TRANSFORM']
const NPM_CDN_BASE = BLOG['NPM_CDN_BASE']
const CDNJS_CDN_BASE = BLOG['CDNJS_CDN_BASE']
const JSDELIVR_CDN_BASE = BLOG['JSDELIVR_CDN_BASE']

// 通用替换函数，根据路径结构进行替换
export function transformCDNUrl(url) {
  // 如果 URL 以 jsdelivr.net 开头，直接替换为镜像 URL
  if (JSDELIVR_CDN_BASE && url.startsWith('https://cdn.jsdelivr.net/')) {
    return url.replace('https://cdn.jsdelivr.net/', JSDELIVR_CDN_BASE)
  }

  // 定义 CDN 路径的匹配规则
  const cdnPathPatterns = [
    {
      // 匹配 npm 或 unpkg 结构：/npm/package@version/file.js 或 /package@version/file.js
      base: NPM_CDN_BASE,
      regex:
        /^https?:\/\/[^/]+\/(?:npm\/)?([^\s'"]+@\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,
      transform: (match, packagePath, filePath) =>
        `${NPM_CDN_BASE}${packagePath.split('@')[0]}/${packagePath.split('@')[1]}/files/${filePath}`
    },
    {
      // 匹配 CDNJS 结构：/ajax/libs/library/version/file.js
      base: CDNJS_CDN_BASE,
      regex:
        /^https?:\/\/[^/]+\/ajax\/libs\/([^\s'"]+)\/(\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,
      transform: (match, libName, version, filePath) =>
        `${CDNJS_CDN_BASE}ajax/libs/${libName}/${version}/${filePath}`
    }
  ]

  // 遍历所有 CDN 路径模式并尝试匹配和转换
  for (const pattern of cdnPathPatterns) {
    // 只有当 URL 不以指定的 base 开头时才进行正则匹配
    if (pattern.base && !url.startsWith(pattern.base)) {
      // 如果正则匹配成功，则进行转换
 ...
 * @param type js 或 css
 * @param cdnTransform 是否转换CDN地址
 * @returns {Promise<unknown>}
 */
export function loadExternalResource(url, type = 'js', cdnTransform = false) {
  if (CDN_TRANSFORM || cdnTransform) {
    console.log('Replacing from CDN URL: ' + url)
    url = transformCDNUrl(url)
    console.log('Replacing CDN URL to: ' + url)
  }
  // 检查是否已存在
  const elements =
    type === 'js'

...
function transformCDNUrl(url) {
  ...
  // 如果 URL 包含 jsdelivr.net，则直接替换为镜像 URL
  if (url.includes('jsdelivr.net')) {
    return url.replace('https://cdn.jsdelivr.net/', JSDELIVR_CDN_BASE)
  }

  // 定义常见 CDN 路径结构的正则表达式和转换规则，基于基础 URL 是否存在来决定是否匹配和转换
  const cdnPathPatterns = [
    {
      // 匹配 npm 结构，如 /npm/package@version/file.js 和 unpkg 结构，如 /package@version/file.js
      shouldApply: !!NPM_CDN_BASE, // 只有 npmCdnBase 存在时才匹配和转换
      regex:
    ...
    {
      // 匹配 CDNJS 结构，如 /ajax/libs/library/version/file.js
      shouldApply: !!CDNJS_CDN_BASE, // 只有 cdnJSCdnBase 存在时才匹配和转换
      regex:
  ...
  for (const pattern of cdnPathPatterns) {
    // 先检查是否需要应用此匹配模式
    if (pattern.shouldApply && pattern.regex.test(url)) {
      url = url.replace(pattern.regex, pattern.transform)
      return url
    }
  }
  return url // 不符合的 URL 保持不变
}
...
export function loadExternalResource(url, type = 'js', transform = true) {
  if (transform) {
    console.log('Replacing cdnjs URL: ' + url)
    url = transformCDNUrl(url)
    console.log('Replacing cdnjs URL to: ' + url)
  }
  // 检查是否已存在
  const elements =
    type === 'js'

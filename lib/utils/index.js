...
// 通用替换函数，根据路径结构进行替换
function transformCDNUrl(targetCdnBase, url) {
  // 定义常见 CDN 路径结构的正则表达式，基于路径结构进行匹配和替换
  const cdnPathPatterns = [
    {
      // 匹配 npm 结构，如 /npm/package@version/file.js
      regex:
        /^https?:\/\/[^/]+\/npm\/([^\s'"]+@\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,
      transform: (match, packagePath, filePath) =>
        `${targetCdnBase}${packagePath.split('@')[0]}/${packagePath.split('@')[1]}/${filePath}`
    },
    {
      ...
      transform: (match, libName, version, filePath) =>
        `${targetCdnBase}${libName}/${version}/${filePath}`
    },
    {
      // 匹配 jsDelivr 结构，如 /gh/username/repo@version/file.js
      regex:
        /^https?:\/\/[^/]+\/gh\/([^\/]+\/[^@]+)@([\d.]+)\/([^\s'"]+\.(js|css))$/,
      transform: (match, repoPath, version, filePath) =>
        `${targetCdnBase}${repoPath.split('/')[1]}/${version}/${filePath}`
    },
    {
      // 匹配通用的 unpkg 结构，如 /package@version/file.js
      regex:
        /^https?:\/\/[^/]+\/([^\s'"]+@\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,
      transform: (match, packagePath, filePath) =>
        `${targetCdnBase}${packagePath.split('@')[0]}/${packagePath.split('@')[1]}/${filePath}`
    }
  ]
  for (const pattern of cdnPathPatterns) {
    if (pattern.regex.test(url)) {
      return url.replace(pattern.regex, pattern.transform)
    }
  }
  return url // 不符合的URL保持不变
}
...
export function loadExternalResource(url, type = 'js', transform = true) {
  // 自定义目标 CDN 基础 URL，可以改成任何你希望的 CDN 地址
  const targetCdnBase = siteConfig('STATIC_RESOURCE_CDN')
  if (targetCdnBase && transform) {
    url = transformCDNUrl(targetCdnBase, url)
  }
  // 检查是否已存在
  const elements =
    type === 'js'

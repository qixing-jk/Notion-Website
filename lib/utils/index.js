...
function transformCDNUrl(url) {
  const NPM_CDN_BASE = siteConfig(
    'NPM_CDN_BASE',
    'https://registry.npmmirror.com/'
  )
  const CDNJS_CDN_BASE = siteConfig(
    'CDNJS_CDN_BASE',
    'https://cdnjs.cloudflare.com/'
  )
  const JSDELIVR_CDN_BASE = siteConfig(
    'JSDELIVR_CDN_BASE',
  ...
  const cdnPathPatterns = [
    {
      ...
      base: NPM_CDN_BASE,
      regex: /^https?:\/\/[^/]+\/(?:npm\/)?([^\s'"]+@\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,
    ...
    },
    {
      ...
      base: CDNJS_CDN_BASE,
      regex: /^https?:\/\/[^/]+\/ajax\/libs\/([^\s'"]+)\/(\d+\.\d+\.\d+)\/([^\s'"]+\.(js|css))$/,

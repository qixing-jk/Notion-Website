...
export async function processPostData(props, from) {
  ...
  if (props.post?.blockMap?.block) {
    // 目录默认加载
    const contents = Object.keys(props.post.blockMap.block).filter(
      key => props.post.blockMap.block[key]?.value?.parent_id === props.post.id
    )
    props.post.toc = getPageTableOfContents(contents, props.post.blockMap)
  ...
  if (Object.values(props.post.blockMap.collection_query)[0]) {
    for (const collectionView in props.post.blockMap.collection_view) {
      if (!Object.keys(Object.values(props.post.blockMap.collection_query)[0]).includes(collectionView)) {

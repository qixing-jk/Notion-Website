...
import getAllViewPageIds from '@/lib/db/notion/getAllViewPageIds'
  ...
  if (isBrowser) {
    ...
    const notionArticleObserver = new MutationObserver((changedNodes) => {
      ...
      if (typeof preActivatedViewIndex !== 'undefined') {
        const elementNodeList = document.querySelector('#article-wrapper #notion-article')?.querySelectorAll('.notion-collection-card-body')
        ...
        if (elementNodeList && elementNodeList?.length === viewsLengthDict[preActivatedViewIndex] && elementNodeList[0].children.length === elementNodeList[elementNodeList.length - 1]?.children?.length) {
          ...
          // 监听View切换，重新渲染
          const viewTabsButton = document.querySelector('#article-wrapper #notion-article').querySelectorAll('.notion-collection-view-tabs-content-item')
...
function getActivatedViewIndex() {
  const activatedViewList = document.querySelector('#article-wrapper #notion-article')?.querySelectorAll('.notion-collection-view-tabs-content-item')
...
function setHrefUrls(galleryViewColumnIndexDict, activatedViewIndex) {
  ...
  const collectionCardBodyList = document.querySelector('#article-wrapper #notion-article')?.querySelectorAll('.notion-collection-card-body')

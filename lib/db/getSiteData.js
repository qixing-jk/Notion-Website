...
export async function getGlobalData({
  ...
  return cleanData ? handleDataBeforeReturn(deepClone(data)) : deepClone(data)
}
...
export function cleanBlock(item) {
  ...
  delete post?.blockMap?.collection_view
  delete post?.blockMap?.collection_query
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      if (pageBlock[i]?.value?.type === 'page') {
        // 清除 Page类型block多余属性
        for (const valueKey in pageBlock[i]?.value) {
          if (
            !new Set(['id', 'type', 'properties', 'format', 'content']).has(
              valueKey
            )
          ) {
            delete pageBlock[i]?.value[valueKey]
          }
        ...
        for (const valueKey in pageBlock[i]?.value.properties) {
          if (valueKey !== 'title') {
            delete pageBlock[i]?.value.properties[valueKey]
          }
        }
      }
 ...
 */
export function cleanDataBeforeReturn(props) {

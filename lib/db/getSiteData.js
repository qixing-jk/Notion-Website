...
export function handleDataBeforeReturn(db) {
  // 清理多余数据
  if (db.collectionView) {
    for (const collectionView in db.post.blockMap.collection_view) {
      if (Object.keys(db.collectionView).includes(collectionView)) {
        delete db.post.blockMap.collection_view[collectionView]
      }
    }
  }
  delete db.post.blockMap.collection[db.collection.id]
  db.post = cleanBlock(db.post)

  delete db.allPages
  delete db.post?.content
  delete db.block
  delete db.schema
